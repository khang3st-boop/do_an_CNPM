...
def run_lightweight_migrations():
    statements = [
        ...
        IF COL_LENGTH(N'dbo.Users', N'Department') IS NULL
            ALTER TABLE dbo.Users ADD Department NVARCHAR(50) NOT NULL
            CONSTRAINT DF_Users_Department DEFAULT N'reception'
        """,
        """
        IF COL_LENGTH(N'dbo.Users', N'Status') IS NULL
            ALTER TABLE dbo.Users ADD Status NVARCHAR(20) NOT NULL
...
def seed_data():
    ...
    try:
        # Seed admin
        auth = AuthService(db)
        user, _ = auth.create_user("admin", "admin123", "admin@hotel.com", "admin", "management")
        if user:
            print("[SEED] Tài khoản admin: admin / admin123")

        # Seed manager
        mgr, _ = auth.create_user("manager1", "manager123", "manager@hotel.com", "manager", "management")
        if mgr:
