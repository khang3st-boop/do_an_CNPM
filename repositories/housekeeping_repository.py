...
class HousekeepingRepository:
    ...
    def get_overdue(self):
        return (
            self._with_relations()
            .filter(
                HousekeepingTask.ScheduledAt < datetime.now(),
                HousekeepingTask.Status.notin_(["done", "cancelled"]),
            )
            .order_by(HousekeepingTask.ScheduledAt.asc())
            .all()
        )
    ...
    def update_status(self, task_id: int, status: str, result_note: str = None):
        t = self.db.query(HousekeepingTask).filter(HousekeepingTask.TaskID == task_id).first()
        if t:
            t.Status = status
            if result_note is not None:
                t.ResultNote = result_note
            if status == "done":
